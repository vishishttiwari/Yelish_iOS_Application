...
- (void)viewWillAppear:(BOOL)animated {
    ...
    self.navigationItem.title = @"Trending Yells";
    
    [self.navigationController.navigationBar setTitleTextAttributes:@{ NSFontAttributeName: [UIFont fontWithName:@"Helvetica Neue" size:25.0f], NSForegroundColorAttributeName: [UIColor whiteColor]
    }];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self.collectionView registerClass:[WorldViewCell class] forCellWithReuseIdentifier:reuseIdentifier];
    
    WorldViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:reuseIdentifier forIndexPath:indexPath];
    ...
    [query getFirstObjectInBackgroundWithBlock:^(PFObject *object, NSError *error) {
        if (error) {
            ...
            if ([temp isEqualToString:@"video"]) {
                [temp2 getDataInBackgroundWithBlock:^(NSData *imageData, NSError *error) {
                    if (!error) {
                        cell.locationImage.image = [UIImage imageWithData:imageData];
                    }
                }];
            } else {
                [temp1 getDataInBackgroundWithBlock:^(NSData *imageData, NSError *error) {
                    if (!error) {
                        cell.locationImage.image = [UIImage imageWithData:imageData];
                    }
                }];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [self performSegueWithIdentifier:@"showImages2" sender:self];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    if ([segue.identifier isEqualToString:@"showSearch"]) {
        ...
        searchViewController.kms = self.kms;
    }
    else if ([segue.identifier isEqualToString:@"showImages2"]) {
        [segue.destinationViewController setHidesBottomBarWhenPushed:YES];
        ...
        collectionViewController.span = 5.0;
    }
}
...
- (void)retrieveLocations {
    ...
    [query findObjectsInBackgroundWithBlock:^(NSArray * _Nullable objects, NSError * _Nullable error) {
        if (error) {
            NSLog(@"error in query: %@", error.description); // todo why is this ever happening?
        }
        else {
